...
Begin VB.Form frmVolLabelTEMP 
   ...
   End
End
Attribute VB_Name = "frmVolLabelTEMP"
...
de_informa.ConsultaAWB xCodAwb
    ...
    TxtVolumes.Text = TxtVolumesAWB.Text
    TxtVolumes.SetFocus
    Else
...
Print #1, ""
    ...
    Print #1, "Cia. Aerea: " & Trim(UCase(lblFantasiaIns.Caption))
    Print #1, "AWB: " & Trim(TxtCodAWB.Text)
    Print #1, "EXPEDIDOR: " & UCase(Trim(TxtExpedidor.Text)) & " - " & UCase(Trim(TxtCidadeOrigem.Text)) & " - " & Trim(TxtSiglaOrigem.Text)
    Print #1, "DESTINATARIO: " & UCase(Trim(TxtDestinatario.Text)) & " - " & UCase(Trim(TxtCidadeDestino.Text)) & " - " & Trim(TxtSiglaDestino.Text)
    Print #1, "PESO: " & Trim(TxtPesoTotal.Text)
    Print #1, "VOLUMES: " & Trim(Str(xCont)) & "/" & Trim(Str(Val(TxtVolumes.Text)))
    Print #1, ""
    Print #1, ""
    Print #1, ""
    Next
...
Dim xNomeFilial As String
Dim xSigla As String
Dim xNomeCia As String

xFilial = TxtFilial.Caption
xNomeFilial = TxtNomeFilial.Caption
xSigla = TxtSigla.Text
xNomeCia = lblFantasiaIns.Caption
Call LimpaTela(Me)
TxtFilial.Caption = xFilial
TxtNomeFilial.Caption = xNomeFilial
TxtSigla.Text = xSigla
lblFantasiaIns.Caption = xNomeCia
TxtAWB.SetFocus
