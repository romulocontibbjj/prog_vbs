...
Begin VB.Form frmExportEDI 
   ...
   Begin VB.Frame Frame11 
      Caption         =   "Arquivo para VIDEOLAR (Prévia de Faturamento)"
      BeginProperty Font 
         Name            =   "MS Sans Serif"
         Size            =   8.25
         Charset         =   0
      ...
      Begin VB.Frame Frame12 
         ...
         Top             =   240
         Width           =   3975
         Begin MSMask.MaskEdBox mskDataVL2 
            Height          =   285
            Left            =   2280
            TabIndex        =   84
            Top             =   360
            ...
            PromptChar      =   "_"
         End
         Begin MSMask.MaskEdBox mskDataVL1 
            Height          =   285
            Left            =   600
            TabIndex        =   82
            Top             =   360
         ...
         End
      End
      Begin VB.CommandButton cmdGeraVL 
      ...
      Begin VB.Label Label32 
         Caption         =   "Arquivo EDI Gerado no Diretório C:\INFORMA"
         Height          =   375
         Left            =   120
         TabIndex        =   85
         Top             =   1080
...
Private Sub cmdGeraBomi_Click()
    ...
    Else
        ...
        Do Until de_informa.rsSel_BossCTC.EOF
            ...
            '31/05/05 - Romulo - Testando datas
            If Len(Trim$(xdtctc)) < 6 Then
                MsgBox "ERRO DE DATA: " & xdtctc
            End If
            ...
            End If
                                        ...
                                        'TESTANDO EDWARDS
                'If (Mid$(de_informa.rsSel_BossCTC.Fields("remet_cgc"), 1, 8) = "43078849" Or Mid$(de_informa.rsSel_BossCTC.Fields("remet_cgc"), 1, 8) = "05944604") And Trim$(xdtmnf) = "" Then
                
                 '   MsgBox Mid$(de_informa.rsSel_BossCTC.Fields("remet_cgc"), 1, 8) & Chr$(13) & _
                  '              " DATA CTC: " & xdtctc & Chr$(13) & "DATA ENTREGA: " & xdtmnf
               ' End If
      
            
            If IsNumeric(xdtentrega) = False Then
      ...
      'EDWARDS
            If Mid(de_informa.rsSel_BossCTC.Fields("remet_cgc"), 1, 8) = "05944604" Or Mid(de_informa.rsSel_BossCTC.Fields("remet_cgc"), 1, 8) = "43078849" Then
            
                'MsgBox de_informa.rsSel_BossCTC.Fields("remet_cgc") & Chr$(13) & xdtctc & Chr$(13) & xdtentrega
                
                With de_informa.rsSel_BossCTC
                
                List3.AddItem "CGC:" & .Fields("remet_cgc") & " - CTC:" & .Fields("filialctc") & " - DATA_CTC:" & xdtctc & " - DATA_ENTREGA:" & xdtentrega
            ...
            If Trim$(IsNumeric(xdtmnf)) = False Then
            
                xdtmnf = "000000"
                 List2.AddItem xnf & " - " & xdtmnf
            End If
            
            
            List1.AddItem xnf & " - " & xdtmnf
           
           Me.Refresh
            
            
            If xhsentrega = ":00" Then
                xhsentrega = "000000"
            End If
...
Private Sub cmdGeraVL_Click()
    Dim xlinha As String, xfile As String, xctc As String, xcidade As String, xuf As String
    Dim xpeso As Currency, xvalmerc As Currency, xfretetotal As Currency, xfretetotalbruto As Currency
    Dim xNfs As String, xmodal As String, xdata As String, xicms As Currency, xestudio As String
    
    Me.MousePointer = 11
    
    If de_informa.rsSel_VLBuscaCTCsPer.State = 1 Then de_informa.rsSel_VLBuscaCTCsPer.Close
    de_informa.Sel_VLBuscaCTCsPer CDate(mskDataVL1), CDate(mskDataVL2)
    
    If de_informa.rsSel_VLBuscaCTCsPer.RecordCount < 1 Then
        MsgBox "Não Há Dados para o Período Selecionado !"
        Me.MousePointer = 0
        Exit Sub
    Else
    
        xfile = "INTEC_" & zeros(Day(CDate(mskDataVL1)), 2) & zeros(Month(CDate(mskDataVL1)), 2) & "_" & _
                zeros(Day(CDate(mskDataVL2)), 2) & zeros(Month(CDate(mskDataVL2)), 2) & ".TXT"
                
        lblvltotcont = "de " & Trim$(Str(de_informa.rsSel_VLBuscaCTCsPer.RecordCount))
    
        Open "C:\INFORMA\" & xfile For Output As #1
        
        lblvlcont = 0
        
        Print #1, "CTC#Cidade Dest#UF#Peso#Valor Merc#Frete Liq#Frete Bruto#Notas Fiscais#Modal#Data#ICMS#Estudio#"
    
        Do Until de_informa.rsSel_VLBuscaCTCsPer.EOF
        
            lblvlcont = CDbl(lblvlcont) + 1
            DoEvents
        
            xlinha = ""
            
            xctc = Mid$(de_informa.rsSel_VLBuscaCTCsPer.Fields("filialctc"), 3)
            
            xcidade = Trim$(de_informa.rsSel_VLBuscaCTCsPer.Fields("cidade_dest"))
            xuf = de_informa.rsSel_VLBuscaCTCsPer.Fields("uf_dest")
            xpeso = de_informa.rsSel_VLBuscaCTCsPer.Fields("peso")
            xvalmerc = de_informa.rsSel_VLBuscaCTCsPer.Fields("valmerc")
            xfretetotal = de_informa.rsSel_VLBuscaCTCsPer.Fields("fretetotal")
            xfretetotalbruto = de_informa.rsSel_VLBuscaCTCsPer.Fields("fretetotalbruto")
            xNfs = de_informa.rsSel_VLBuscaCTCsPer.Fields("nfs")
            xmodal = de_informa.rsSel_VLBuscaCTCsPer.Fields("modal")
            xdata = Trim$(Str(Year(de_informa.rsSel_VLBuscaCTCsPer.Fields("data")))) & "/" & _
                    Trim$(zeros(Month(de_informa.rsSel_VLBuscaCTCsPer.Fields("data")), 2)) & "/" & _
                    Trim$(zeros(Day(de_informa.rsSel_VLBuscaCTCsPer.Fields("data")), 2))
            xicms = xfretetotalbruto - xfretetotal
            
            If de_informa.rsSel_NFsdoCTC.State = 1 Then de_informa.rsSel_NFsdoCTC.Close
            de_informa.Sel_NFsdoCTC de_informa.rsSel_VLBuscaCTCsPer.Fields("filialctc")
            ...
            Do Until de_informa.rsSel_NFsdoCTC.EOF
                ...
                de_informa.Sel_NFVideolar de_informa.rsSel_NFsdoCTC.Fields("cliente_cgc"), _
                                          CDbl(de_informa.rsSel_NFsdoCTC.Fields("numnf")), _
                                          de_informa.rsSel_NFsdoCTC.Fields("serie")
                                          
                If de_informa.rsSel_NFVideolar.RecordCount > 0 Then
                    If Len(Trim$(de_informa.rsSel_NFVideolar.Fields("id_local"))) >= 2 Then
                        xestudio = xestudio & de_informa.rsSel_NFVideolar.Fields("id_local") & "/ "
                    Else
                        xestudio = xestudio & ".. / "
                    End If
                Else
            ...
            xlinha = xctc & "#" & xcidade & "#" & xuf & "#" & xpeso & "#" & xvalmerc & "#" & xfretetotal & "#" & _
                     xfretetotalbruto & "#" & xNfs & "#" & xmodal & "#" & xdata & "#" & xicms & "#" & xestudio
            
            Print #1, xlinha
            
            de_informa.rsSel_VLBuscaCTCsPer.MoveNext
...
Private Sub CmdProcessar_Click()
    ...
    If de_informa.rsSel_EDI_Ocorr.RecordCount > 0 Then
        ...
        ElseIf Mid$(txtCGC, 1, 8) = "04490850" Then 'gillette
            xarquivo = "C:\INFORMA\EDI_EXP\GILLETTE\INTEOCO" & zeros(Day(datahora("data")), 2) & zeros(Month(datahora("data")), 2) & ".TXT"
        ...
        ElseIf Mid$(txtCGC, 1, 8) = "04229761" Then  'VIDEOLAR
            xarquivo = "C:\INFORMA\EDI_EXP\VIDEOLAR\EDIOCOINT" & zeros(Day(datahora("data")), 2) & zeros(Month(datahora("data")), 2) & Mid$(Trim$(Str(Year(datahora("data")))), 3, 2) & ".TXT"
        ElseIf Mid$(txtCGC, 1, 8) = "60412327" Then  'ALCON
            xarquivo = "C:\INFORMA\EDI_EXP\ALCON\INTOCOREN" & zeros(Day(datahora("data")), 2) & zeros(Month(datahora("data")), 2) & ".TXT"
...
Open "C:\BOMI_EDWARDS.txt" For Output As #1
...
xNomeFile = Mid$(Trim(UCase(txtCGCRem.Text)), 1, 8) & "_" & zeros(Day(xAgora), 2) & zeros(Month(xAgora), 2) & "_" & zeros(Hour(xAgora), 2) & zeros(Minute(xAgora), 2) & ".txt"
    ...
    Open "C:\INFORMA\EDI_EXP\PROCEDA\" & xNomeFile For Output As #1
