...
Private Sub Timer1_Timer()
    ...
    Do Until EOF(1)
        ...
        If Mid$(xlinha, 1, 3) = "312" Then   'DESTINATÁRIO
            ...
            'BUSCA CADASTRO DE CIDADES - TB_CADCIDADES
            If de_informa.rsSel_CadCidadePorCidadeUF.State = 1 Then de_informa.rsSel_CadCidadePorCidadeUF.Close
            de_informa.Sel_CadCidadePorCidadeUF xdest_cidade, xdest_uf
            
            If de_informa.rsSel_CadCidadePorCidadeUF.RecordCount > 0 Then
            Else
                'NÃO ENCONTROU CIDADE BUSCA POR CEP (8 DIGITOS)
                If de_informa.rsSel_BuscaCidadeUFporCEP.State Then de_informa.rsSel_BuscaCidadeUFporCEP.Close
                de_informa.Sel_BuscaCidadeUFporCEP xdest_cep
                
                If de_informa.rsSel_BuscaCidadeUFporCEP.RecordCount > 0 Then
                    'ENCONTROU POR CEP(8), BUSCA NOVAMENTE PELO NOME DA CIDADE DA TABELA CEP.TB_CADCEP
                    xdest_cidade = de_informa.rsSel_BuscaCidadeUFporCEP.Fields("cidade")
                    xdest_uf = de_informa.rsSel_BuscaCidadeUFporCEP.Fields("uf")
                Else
                    'NÃO ENCONTROU POR CEP(8) TENTA POR CEP 5 DIGITOS
                    If de_informa.rsSel_BuscaCidadeUFporCEP.State Then de_informa.rsSel_BuscaCidadeUFporCEP.Close
                    de_informa.Sel_BuscaCidadeUFporCEP Mid$(xdest_cep, 1, 5) & "%"
                    If de_informa.rsSel_BuscaCidadeUFporCEP.RecordCount > 0 Then
                        'ENCONTROU PELO CEP(5), BUSCA NOVAMENTE PELO NOME DA CIDADE DA TABELA CEP.TB_CADCEP
                        xdest_cidade = de_informa.rsSel_BuscaCidadeUFporCEP.Fields("cidade")
                        xdest_uf = de_informa.rsSel_BuscaCidadeUFporCEP.Fields("uf")
                    Else
                        xgrava = "N"
                        de_informa.Ins_LogNotFis "CRITICA DADO", "NF " & xNumNF & " CIDADE/UF INVÁLIDO: " & xdest_cidade + "/" + xdest_uf, lblArquivo.Caption
                    End If
                End If
            End If
